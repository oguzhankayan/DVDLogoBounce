...
Corner is sold as an **ambient visualiser / customizable retro screensaver /
...
> banner for a clean stream background.
>
> **HONEST** — Paid once. No subscription. No ads. No account. No tracking — your
> statistics never leave your Apple TV.
>
> 60 fps, lightweight, low‑memory. Just leave it on.
...
- Cross‑link from any companion site/social with `?app=…` campaign tokens to read
  acquisition without an in‑app SDK.

## App icon direction

tvOS layered "App Icon & Top Shelf Image" (parallax). 2–3 layers, dark, premium,
unmistakable from across the room — no skeuomorphic DVD case, no joke.
...
- **App Store icon** (`1280×768`, flattened render of the same composition) — same,
  with a touch more contrast and a subtle "+1" spark to imply the counter.
- Keep it monochrome‑plus‑one‑accent so it works on any tvOS home‑screen wallpaper.
  Provide `Top Shelf Image` (`1920×720`) and `Top Shelf Image Wide` (`2320×720`) as
  a wide, calm version of the same scene (logo mid‑drift, generous negative space,
  the wordmark small in a corner).

## Screenshots direction (Apple TV, 1920×1080 / 3840×2160)
...
These are *designed* for, not all shipped — see ROADMAP. None of them require an
account; anything networked is opt‑in and sends no identifiers.

- **Daily impossible seed** — every device that opts in runs the *same* starting
  layout on a given UTC day (the engine is already deterministic via
  `SeededRandom.dailySeed()`); compare your "corners on today's seed" with the
  number people post.
